...
- **Reports** — a column chart of each month against the previous year and a doughnut chart of spending per category, with the exact numbers in tables below. Charts follow the light/dark theme and the chosen language; status colours (overdue, due soon…) have separate light and dark shades, all with readable contrast.
...
- **Export to Excel (CSV)** — **Export…** on Bills (the list as filtered) and on Reports (the year by month and by category). Column names are in the app language; numbers, dates and the column separator follow your Windows regional settings, so Excel opens the file correctly.
