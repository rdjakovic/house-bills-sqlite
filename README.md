...
- **Export to Excel (CSV)** — **Export…** on Bills (the list as filtered) and on Reports (the year by month and by category, for the chosen recurring bill if one is picked). Column names are in the app language; numbers, dates and the column separator follow your Windows regional settings, so Excel opens the file correctly.
