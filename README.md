...
- **Overview** (start page) — what's overdue and due in the next 7 days (with a shortcut to those bills), this month's total and how it compares with last month, and the next bills to pay.
...
- **Recurring bills** — templates that repeat weekly, monthly, quarterly or yearly, with an optional end date. For bills whose amount changes each time (electricity, water), tick **Amount varies**: new bills are created as estimates (shown as "≈ 4.230,50 RSD") based on the last actual amount; enter the real amount when the bill arrives (untick **Estimated amount**), or simply record the payment — the amount paid becomes the bill's amount. Bills are generated automatically for the next 31 days when the app starts (and on demand). Pausing and resuming a template doesn't back-fill the paused period; deleting a generated bill doesn't bring it back.
...
- **Reports** — a column chart of each month against the previous year and a doughnut chart of spending per category, with the exact numbers in tables below. Pick a recurring bill (e.g. electricity) to see only that bill's months, totals and year-on-year change. Charts follow the light/dark theme and the chosen language; status colours (overdue, due soon…) have separate light and dark shades, all with readable contrast.
...
- **Export to Excel (CSV)** — **Export…** on Bills (the list as filtered), on Reports (the year by month and by category, for the chosen recurring bill if one is picked), and on Payees and Categories (the lists). Column names are in the app language; numbers, dates and the column separator follow your Windows regional settings, so Excel opens the file correctly.
- **Import from Excel (CSV)** — **Import…** on Bills, Payees and Categories adds the rows of a CSV file; the easiest template is an export, edited in Excel (saved as *CSV UTF-8* or plain *CSV*). Columns are recognised by their English or Serbian names in any order; others (such as Status) are ignored. Payees and categories a bills file names are created if they don't exist. Anything already in the list is skipped — a payee or category with the same name, a bill with the same due date, description, payee and amount — so importing the same file twice adds nothing the second time. If any row has a problem (an empty description, "4230.50" where your settings expect "4230,50"…), nothing is imported and you get a list of the rows to fix.
